...
function drawShape() {
    ...
    values.raduis *= values.raduisGrowthFactor;
    
    shape = template.clone();
    shape.x = Math.cos(values.rotation * Math.PI / 180) * values.raduis + (canvas.width / 2);
    shape.y = Math.sin(values.rotation * Math.PI / 180) * values.raduis + (canvas.height / 2);
